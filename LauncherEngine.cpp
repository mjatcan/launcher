...
Launcher::Launcher()
{
    ...
    m_bPatchesGot = false;
    m_iVersion = 0;
    m_stateString = xorstr("AlfaACS Version Kontrol Ediliyor ve Oyun Baslatilmaya Hazirlaniyor.");

    const size_t IPSize = 256;
    char* sIP = new char[IPSize];
    GetCurrentDirectoryA(MAX_PATH, WorkingPath);
    GetPrivateProfileStringA(xorstr("Server"), xorstr("IP0"), xorstr("176.98.43.221"), sIP, IPSize, (std::string(WorkingPath) + xorstr("\\Server.ini")).c_str()); // AlfaACS LISANS
    ...
    if (KeyExists(HKEY_LOCAL_MACHINE, "SOFTWARE\\AlfaACS\\PATH"))
    {
        std::string path = GetVal(HKEY_LOCAL_MACHINE, "SOFTWARE\\AlfaACS\\PATH");
    ...
    if (!alreadyExists)
    {
        ...
        std::vector<std::string> outs = { "KnightOnLine.exe", "AlfaACS", "Launcher.exe" };
        ...
        if (!KeyExists(HKEY_LOCAL_MACHINE, "SOFTWARE\\AlfaACS\\PATH"))
        {
            CreateKey(HKEY_LOCAL_MACHINE, "SOFTWARE\\AlfaACS\\PATH");
        }
        SetVal(HKEY_LOCAL_MACHINE, "SOFTWARE\\AlfaACS\\PATH", m_base);
    }
}
...
bool Launcher::Start()
{
    ...
    if (iErr)
    {
        m_stateString = xorstr("AlfaACS Baglanti hatasi. Lutfen Tekrar Deneyiniz.");
        return false;
    }
...
int ProgCallback(void* ptr, double dTotalToDownload, double dNowDownloaded, double dTotalToUpload, double dNowUploaded)
{
    ...
    Engine->SetState(std::format("AlfaACS Dosyalari Indiriliyor {}: {:.2f}/{:.2f} MB.", Engine->m_currentFile.c_str(), parseMB(dNowDownloaded), parseMB(dTotalToDownload)));
    return 0;
}
...
int on_extract_entry(const char *filename, void *arg) {
	Engine->SetState(std::format("AlfaACS Dosyalari Cikartiliyor: {}", filename));
	return 0;
}
...
bool Launcher::DownloadPatch(std::string server, std::string path, std::string file)
{
    ...
    {
        MessageBoxA(NULL, xorstr("KnightOnLine.exe Kapatip tekrar deneyiniz"), "AlfaACS", MB_ICONEXCLAMATION);
        ExitProcess(0);
        return false;
    }
...
void Launcher::Download()
{
	if (m_settingsVersion > m_iVersion)
	{
		SetState(xorstr("Version Gecersiz."));
		ready = false;
		return;
	}
...
bool Launcher::HandlePacket(Packet& pkt)
{
	...
	case 0x2:
	{
        ...
        SetState(xorstr("AlfaACS Dosyalari Paketleniyor..."));
        CHDRSystem* hdrPacker = new CHDRSystem;
        hdrPacker->Pack();
        delete hdrPacker;
        Engine->SetPercent(100);
		SetState(xorstr("AlfaACS Guncelleme Islemi Tamamlandi..."));
		ready = true;
	}
	break;
    case 0x3:
